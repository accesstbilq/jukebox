...
def get_duration_sec(file, cache=False):
    try:
        ...
        try:
            ...
            print(f"Skipping file due to UnicodeDecodeError: {file}")
            return None
...
def load_audio(file, sr, offset, duration, resample=True, approx=False, time_base='samples', check_duration=True):
    ...
    container = av.open(file)
    audio = container.streams.get(audio=0)[0]  # Only the first audio stream
    ...
    offset = int(offset / sr / float(audio.time_base))  # Convert offset to time_base units
    duration = int(duration)  # Keep duration in samples
    ...
    for frame in container.decode(audio=0):
        if frame is not None:
            if resample:
                frame.pts = None  # Resetting PTS necessary for resampling
                frame = resampler.resample(frame)
            frame_array = frame.to_ndarray(format='fltp')  # Convert to floats
            read = frame_array.shape[-1]
            if total_read + read > duration:
                read = duration - total_read
            sig[:, total_read:total_read + read] = frame_array[:, :read]
            total_read += read
