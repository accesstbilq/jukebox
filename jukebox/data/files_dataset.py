...
class FilesAudioDataset(Dataset):
    ...
    def get_song_chunk(self, index, offset, test=False):
        ...
        try:
            ...
            if data.shape != (self.channels, self.sample_length):
                raise ValueError(f'Incorrect data shape for file {filename}. Expected {(self.channels, self.sample_length)}, got {data.shape}')
        except Exception as e:
            print(f"Error processing file {filename}: {e}. Skipping this sample.")
            # Optionally, return a silent data array instead of skipping:
            # data = np.zeros((self.channels, self.sample_length), dtype=np.float32)
            return None  # or return data if you chose to use silent array

        if self.labels:
            artist, genre, lyrics = self.get_metadata(filename, test)
            labels = self.labeller.get_label(artist, genre, lyrics, self.durations[index], offset)
    ...
    def __getitem__(self, item):
        try:
            ...
            if data is None:  # Handling None returned by get_song_chunk
                raise ValueError("Received None from get_song_chunk, skipping batch.")
            return data
        except Exception as e:
            print(f"Skipping item {item} due to error: {e}")
            return None  # Returning None to indicate this sample should be skipped
